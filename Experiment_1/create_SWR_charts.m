%create (early) SWR analysis charts. 
...
R1_SWR_count_pre45 = zeros(length(epochs_t.rat_num),1);
R2_SWR_count_pre75 = R1_SWR_count_pre45;
R1_SWR_count_total = R1_SWR_count_pre45;
R2_SWR_count_total = R1_SWR_count_pre45;

%add columns to table
epochs_t = addvars(epochs_t,R1_SWR_count_pre45,R1_SWR_count_total,...
                   R2_SWR_count_pre75,R2_SWR_count_total,'After','R2_SWS_total');

%fill SWR count columns
for i = 1:length(epochs_t.rat_num) %for each epochs.mat file
    ...
    %total columns
    epochs_t.R1_SWR_count_total(i) = size(epochs.tsswr1,1);
    epochs_t.R2_SWR_count_total(i) = size(epochs.tsswr2,1);
    
    %trim SWRs past time limit of epochs
    %note: rename vars later maybe
    R1_ts_temp = trim_to_duration(epochs.sleep1(1),epochs.tsswr1,45);
    R2_ts_temp = trim_to_duration(epochs.sleep2(1),epochs.tsswr2,75);

    %pre columns
    epochs_t.R1_SWR_count_pre45(i) = size(R1_ts_temp,1);
    epochs_t.R2_SWR_count_pre75(i) = size(R2_ts_temp,1);

end

%save chart 
% save('Y:\Seth_temp\Thesis recordings\epochs_table_with_SWS_dur_SWRs.mat','epochs_t')
save('Y:\Seth_temp\Thesis recordings\epochs_table_with_SWS_dur_SWRs_REM_removed.mat','epochs_t')

%%
%%create plots
%note: the following section is largely the same as plot creation is in
%create_SWS_charts and create_spindle_charts
...
x = reshape(epochs_t.R1_SWR_count_pre45,15,[])'; %format durations into a 6x15 (# of rats by 15 days) matrix
x = circshift(x,-1,2); %move probe trial results in column 1 to last column instead and shift eveything else left

y = reshape(epochs_t.R2_SWR_count_pre75,15,[])'; %format durations into a 6x15 (# of rats by 15 days) matrix
...
title("pre-task sleep mean SWR rates for all rats (SEM)")
...
% title("pre-task sleep Group SWR rates (STD)")
...
legend('Control','Experimental')
title("pre-task sleep group SWR rates (SEM)")
...
title("post-task sleep SWR rates for all rats (SEM)")
...
% legend('Control','Experimental')
% title("post-task sleep Group SWR rates (STD)")
...
legend('Control','Experimental')
title("post-task sleep group SWR rates (SEM)")

%full chart formatting
title(charts,"Comparison of SWR rates")
linkaxes([ax1,ax2,ax3,ax4,ax5,ax6],'y')
xlabel(charts,'day','FontSize',14)
ylabel(charts,'mean SWRs per minute of SWS','FontSize',14)
