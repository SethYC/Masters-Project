...
for i in ${!info_array[@]}; do
	...
	then
		...
		if [ $day_num -gt 3 ] #3 days of re-training
		then
			let day_num=day_num-3 
			day_type='Training'
		else
			day_type='Re-Training' 
		fi
	...
	then
		...
		cp $file_name $new_location/Thesis\ recordings/Experiment\ 2/rat${rat_id}/rat${rat_id}_${day_type}_day${day_num}_cam1.avi &
		...
		cp $file_name $new_location/Thesis\ recordings/Experiment\ 2/rat${rat_id}/rat${rat_id}_${day_type}_day${day_num}_cam2.avi &
		#mv -i $file_name $new_location/$(date +%F)/${day_num}_cam1.avi
