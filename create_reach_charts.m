...
sem_omitNaN = @(x) std(x,'omitnan')/sqrt(size(x,2)); %standard error of the mean, although std is preferred in graphs based on https://doi.org/10.4103/2229-3485.100662
my_sem = @(x) std(x)/sqrt(size(x,2));
...
figure; hold on;
rectangle('Position',[14.5 0 1 20], 'EdgeColor','none','FaceColor',[0.8196    0.9490    0.9216])
text(15,-1,'probe','HorizontalAlignment','center','FontSize', 11)
ylim([0 20])
xlim([1 15.5])
ylabel("# successes")
xlabel('day')
